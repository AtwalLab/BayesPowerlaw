...
class BayesPowerlawError(Exception):
    """
    Class used by BayesPowerlaw to handle errors.
    parameters
    ----------
    message: (str)
        The message passed to check(). This only gets passed to the
        BayesPowerlawError constructor when the condition passed to check() is
        False.
    """
...
def check(condition, message):
    ...
    """
    Checks a condition; raises a BayesPowerlawError with message if condition
    evaluates to False
    parameters
    ----------
    condition: (bool)
        A condition that, if false, halts BayesPowerlaw execution and raises a
        clean error to user
    message: (str)
    ...
    if not condition:
        raise BayesPowerlawError(message)

def handle_errors(func):
    """
    Decorator function to handle BayesPowerlaw errors.
    ...
    def wrapped_func(*args, **kwargs):
        ...
        except BayesPowerlawError as e:
