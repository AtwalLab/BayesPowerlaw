...
for i in range(len(exponent)):
    for j in range(sim_number):
        #simulate data
        data = pl.power_law(exponent[i], xmax, sample_size, discrete=disc)

        #Maximum likelihood fit.
        ML_fit = pl.Fit(data, discrete=disc)
        ML_mean[i, j] = np.mean(ML_fit.Guess())

        #Bayes fits jeffrey and flat
        Bayes_jeffrey = pl.Fit_Bayes(data, discrete=disc)
        gamma_j,weight_j=Bayes_jeffrey.posterior()
        Bayes_mean_jeffrey[i, j] = np.mean(gamma_j[0])

        Bayes_flat = pl.Fit_Bayes(data, discrete=disc,prior='flat')
        gamma_f, weight_f = Bayes_flat.posterior()
        Bayes_mean_flat[i, j] = np.mean(gamma_f[0])

        #Linear regression
        if disc:
            x_f, y_f = np.unique(data, return_counts=True)
        else:
...
df.to_csv('method_compare_'+sample_size+'_'+disc+'.txt', sep='\t')
