...
for x in range(10):
    Fit1 = pl.Fit_Bayes(mixed1, niters=10000, mixed=1)
    fits1[x] = np.mean(Fit1.samples)
    fits1_std[x] = np.std(Fit1.samples)
...
plt.savefig('mixed1_n{}-{}.png'.format(int(sample_size1), int(sample_size2)))
plt.savefig('mixed1_n{}-{}.svg'.format(int(sample_size1), int(sample_size2)))
